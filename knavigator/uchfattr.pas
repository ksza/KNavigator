...
interface
  ...
  const cursor = Chr(26);
        ...
        TxtColor: byte = White;
        t: array[1..4]of string[8] = ('ReadOnly', 'Hidden', 'SysFile',
                                      'Archive');
        width = 26;
        height = 12;
  type inregistrare=record
       ...
       TChFAttr=object(TBorderedWin)
         constructor init(ITopx, ITopy: integer; Fisier: string);
...
implementation
  constructor TChFAttr.init;
    ...
    begin
      ...
      textcolor(TxtColor); gotoxy(5, 1); write(' Atribute fisier ');
      assign(F, Fisier);
      GetFAttr(F, FileAttr);

      TipFisier[1].tip:=ReadOnly;
      if ((FileAttr and ReadOnly) <> 0) then TipFisier[1].selectat:=true
      else TipFisier[1].selectat:=false;
      TipFisier[2].tip:=Hidden;
      if ((FileAttr and Hidden) <> 0) then TipFisier[2].selectat:=true
      else TipFisier[2].selectat:=false;
      TipFisier[3].tip:=SysFile;
      if ((FileAttr and SysFile) <> 0) then TipFisier[3].selectat:=true
      else TipFisier[3].selectat:=false;
      TipFisier[4].tip:=Archive;
      if ((FileAttr and Archive) <> 0) then TipFisier[4].selectat:=true
      else TipFisier[4].selectat:=false;

      for i:=2 to Width - 1 do begin
        gotoxy(i, 7); write(Char(LO));
      end;
  ...
  function TChFAttr.GetFileTypes: word;
    var lin, col, i: byte;
        c: char;
